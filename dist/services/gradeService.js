...
exports.evaluateDiscipline = evaluateDiscipline;
exports.evaluateSemester = evaluateSemester;
const GRADE_WEIGHTS = {
    av1: 0.25,
    av2: 0.25,
    av3: 0.3,
...
function requiredScoreForApproval(weightedSum, missingWeight) {
    if (missingWeight <= 0)
        return null;
    return (7 - weightedSum) / missingWeight;
}
function evaluateDiscipline(discipline) {
    const missingAssessments = [];
    let weightedSum = 0;
    let filledWeight = 0;
    ["av1", "av2", "av3", "edag"].forEach((key) => {
    ...
    const maxAchievable = Number((weightedSum + Math.max(missingWeight, 0) * 10).toFixed(2));
    const absenceRisk = discipline.absences >= limitAbsences * 0.8;
    const reprovFalta = discipline.absences > limitAbsences;
    const partialAverage = filledWeight > 0 ? Number((weightedSum / filledWeight).toFixed(2)) : null;
    ...
    if (reprovFalta) {
        ...
        const riskNota = (requiredScore !== null && requiredScore >= 7) ||
...
function evaluateSemester(disciplines) {
    ...
    const weightedAverage = totalWorkload > 0
        ? averages.reduce((acc, cur) => acc + cur.avg * cur.workload, 0) /
            totalWorkload
        : null;
    const average = weightedAverage !== null && !Number.isNaN(weightedAverage)
    ...
    if (allApproved)
        approved = true;
    else if (hasReprovFalta || hasReprovNota)
        approved = false;
